module.exports = app => {
  class TableinfoService extends app.Service {
    * index() {
      ...
      for(let i in record){
        result.push(record[i][`Tables_in_${this.app.config.mysql.client.database}`]);
      }
      return result;
    }
    * show(params) {
    ...
    * create(request) {
      ...
      for (let i = 0; i < requestData.length; i++) {
        ...
        let tempIncre = requestData[i]['Extra']=='auto_increment'?'AUTO_INCREMENT':'';
        queryStr = queryStr + requestData[i]['Field'] + ' ' + requestData[i]['Type'] + ' ' + tempNull + ' ' + tempIncre + ', ';
      ...
      queryStr = queryStr + 'PRIMARY KEY ( '+primaryKey+' ))ENGINE=InnoDB DEFAULT CHARSET=utf8;';
      let record =  yield this.app.mysql.query(queryStr, '');
      return record;
    }
    * destroy(params) {
